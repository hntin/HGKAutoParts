 ...
 * @author HNTIN
 */
public class QuanLyApDungActionSupport extends ActionSupport {
...
//</editor-fold>
    ...
    public QuanLyApDungActionSupport() {
    }
    ...
    public String execute() throws Exception {
        ...
        if (selectedMaSanPham != null && !selectedMaSanPham.equalsIgnoreCase("")) {
            // 0: Không chọn SP nào --> Lấy các áp dụng của tất cả các sản phẩm trong danh sách
            if (selectedMaSanPham.equals("0")) {
                dsSanPhamDongXeDTO = null;
            }
            else { // Lấy các áp dụng của sản phẩm được chọn
                this.sanphamDTO = sanphamBO.getSanPhamDTO(selectedMaSanPham);
                dsSanPhamDongXeDTO = spdxBO.getDSSanPhamDongXeTuongThich(selectedMaSanPham);
            }
